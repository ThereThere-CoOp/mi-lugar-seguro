...
func _on_click() -> void:
	...
	await C.player.say("Es una foto de Ed, Lufi, Sauron y mia.")
	await C.player.say("Nos la tomamos en las navidades pasadas.")
	
	_execute_puzzle_action()
...
func _execute_puzzle_action() -> void:
	if not C.player.state.examined_bedroom_family_canvas:
		C.player.state.examined_bedroom_family_canvas = true
		C.player.state.increase_relationship_level(1)
